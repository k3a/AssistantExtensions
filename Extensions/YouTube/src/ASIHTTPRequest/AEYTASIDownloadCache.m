...
#import "AEYTASIDownloadCache.h"
#import "AEYTASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static AEYTASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface AEYTASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation AEYTASIDownloadCache

+ (void)initialize
{
	if (self == [AEYTASIDownloadCache class]) {
...
- (id)init
{
	...
	[self setDefaultCachePolicy:AEYTASIUseDefaultCachePolicy];
	...
	if (!sharedCache) {
		@synchronized(self) {
			if (!sharedCache) {
				...
				[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"AEYTASIHTTPRequestCache"]];
			}
		}
	}
	return sharedCache;
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:AEYTASICacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (void)updateExpiryForRequest:(AEYTASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-AEYTASIHTTPRequest-Expires"];
...
{
  return [AEYTASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}
...
- (void)storeResponseForRequest:(AEYTASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & AEYTASIDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
	...
	// Create a special 'X-AEYTASIHTTPRequest-Expires' header
	...
	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-AEYTASIHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-AEYTASIHTTPRequest-Response-Status-Code"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(AEYTASIHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == AEYTASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)isCachedDataCurrentForRequest:(AEYTASIHTTPRequest *)request
{
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-AEYTASIHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-AEYTASIHTTPRequest-Expires"];
		if (expires) {
...
{
	[[self accessLock] lock];
	AEYTASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(AEYTASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = AEYTASIAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(AEYTASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == AEYTASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(AEYTASIHTTPRequest *)request
{
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/AEYTASIHTTPRequest/ is cached the same as http://allseeing-i.com/AEYTASIHTTPRequest
...
- (BOOL)canUseCachedDataForRequest:(AEYTASIHTTPRequest *)request
{
	...
	if ([request cachePolicy] & AEYTASIDoNotReadFromCacheCachePolicy) {
		return NO;

	// If we don't want to load the request whatever happens, always pretend we have cached data even if we don't
	} else if ([request cachePolicy] & AEYTASIDontLoadCachePolicy) {
		return YES;
	}
	...
	if ([request cachePolicy] & AEYTASIOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & AEYTASIFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & AEYTASIAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & AEYTASIAskServerIfModifiedCachePolicy) {
