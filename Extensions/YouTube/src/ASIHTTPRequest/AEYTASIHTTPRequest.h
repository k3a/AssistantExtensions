...
@class AEYTASIDataDecompressor;

extern NSString *AEYTASIHTTPRequestVersion;
...
typedef enum _ASIAuthenticationState {
	AEYTASINoAuthenticationNeededYet = 0,
	AEYTASIHTTPAuthenticationNeeded = 1,
	AEYTASIProxyAuthenticationNeeded = 2
} AEYTASIAuthenticationState;

typedef enum _ASINetworkErrorType {
    AEYTASIConnectionFailureErrorType = 1,
    AEYTASIRequestTimedOutErrorType = 2,
    AEYTASIAuthenticationErrorType = 3,
    AEYTASIRequestCancelledErrorType = 4,
    AEYTASIUnableToCreateRequestErrorType = 5,
    AEYTASIInternalErrorWhileBuildingRequestType  = 6,
    AEYTASIInternalErrorWhileApplyingCredentialsType  = 7,
	AEYTASIFileManagementError = 8,
	AEYTASITooMuchRedirectionErrorType = 9,
	AEYTASIUnhandledExceptionError = 10,
	AEYTASICompressionError = 11
	
} AEYTASINetworkErrorType;


// The error domain that all errors generated by AEYTASIHTTPRequest use
...
extern unsigned long const AEYTASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^AEYTASIBasicBlock)(void);
typedef void (^AEYTASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^AEYTASISizeBlock)(long long size);
typedef void (^AEYTASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^AEYTASIDataBlock)(NSData *data);
#endif

@interface AEYTASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the AEYTASIHTTPRequestDelegate protocol
	id <AEYTASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but AEYTASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <AEYTASIHTTPRequestDelegate, AEYTASIProgressDelegate> queue;
	...
	// Automatically set to true in AEYTASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when AEYTASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = AEYTASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <AEYTASIProgressDelegate> uploadProgressDelegate;
	...
	id <AEYTASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force AEYTASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, AEYTASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, AEYTASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - AEYTASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	AEYTASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in AEYTASINetworkQueue.h
	...
	// Tells AEYTASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let AEYTASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// AEYTASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See AEYTASIAuthenticationState values above. 0 == default == No authentication needed yet
	AEYTASIAuthenticationState authenticationNeeded;
	
	// When YES, AEYTASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops AEYTASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be AEYTASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [AEYTASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <AEYTASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See AEYTASICacheDelegate.h for possible values
	AEYTASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See AEYTASICacheDelegate.h for possible values
	AEYTASICacheStoragePolicy cacheStoragePolicy;
	...
	AEYTASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using AEYTASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	AEYTASIHTTPRequest *PACFileRequest;
	...
	AEYTASIBasicBlock startedBlock;

	//block to execute when headers are received
	AEYTASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	AEYTASIBasicBlock completionBlock;

	//block to execute when request fails
	AEYTASIBasicBlock failureBlock;

	//block for when bytes are received
	AEYTASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	AEYTASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	AEYTASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	AEYTASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	AEYTASIDataBlock dataReceivedBlock;

	//block for handling authentication
	AEYTASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	AEYTASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    AEYTASIBasicBlock requestRedirectedBlock;
	#endif
}
...
// Used internally to record bandwidth use, and by AEYTASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, AEYTASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) AEYTASIHTTPRequest *mainRequest;
...
@property (assign, readonly) AEYTASIAuthenticationState authenticationNeeded;
...
@property (assign) id <AEYTASICacheDelegate> downloadCache;
@property (assign) AEYTASICachePolicy cachePolicy;
@property (assign) AEYTASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) AEYTASIDataDecompressor *dataDecompressor;
