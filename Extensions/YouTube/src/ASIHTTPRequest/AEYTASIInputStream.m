...
@implementation AEYTASIInputStream

+ (void)initialize
{
	if (self == [AEYTASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(AEYTASIHTTPRequest *)theRequest
{
	AEYTASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(AEYTASIHTTPRequest *)theRequest
{
	AEYTASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask AEYTASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([AEYTASIHTTPRequest isBandwidthThrottled]) {
		toRead = [AEYTASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[AEYTASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
