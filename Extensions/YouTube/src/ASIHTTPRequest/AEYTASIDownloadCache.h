...
#import "AEYTASICacheDelegate.h"

@interface AEYTASIDownloadCache : NSObject <AEYTASICacheDelegate> {
	
	// The default cache policy for this cache
	// Requests that store data in the cache will use this cache policy if their cache policy is set to AEYTASIUseDefaultCachePolicy
	// Defaults to AEYTASIAskServerIfModifiedWhenStaleCachePolicy
	AEYTASICachePolicy defaultCachePolicy;
	
	// The directory in which cached data will be stored
	// Defaults to a directory called 'AEYTASIHTTPRequestCache' in the temporary directory
...
// Returns a static instance of an AEYTASIDownloadCache
// In most circumstances, it will make sense to use this as a global cache, rather than creating your own cache
// To make AEYTASIHTTPRequests use it automatically, use [AEYTASIHTTPRequest setDefaultCache:[AEYTASIDownloadCache sharedCache]];
+ (id)sharedCache;
...
@property (assign, nonatomic) AEYTASICachePolicy defaultCachePolicy;
