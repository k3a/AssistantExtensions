...
#import "AEYTASIHTTPRequestConfig.h"

typedef enum _ASIPostFormat {
    AEYTASIMultipartFormDataPostFormat = 0,
    AEYTASIURLEncodedPostFormat = 1
	
} AEYTASIPostFormat;

@interface AEYTASIFormDataRequest : AEYTASIHTTPRequest <NSCopying> {
	...
	AEYTASIPostFormat postFormat;
	
	NSStringEncoding stringEncoding;
	
#if DEBUG_FORM_DATA_REQUEST
	// Will store a string version of the request body that will be printed to the console when AEYTASIHTTPREQUEST_DEBUG is set in GCC_PREPROCESSOR_DEFINITIONS
...
@property (assign) AEYTASIPostFormat postFormat;
